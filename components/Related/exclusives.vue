<template>
  <v-sheet class="mx-auto sliderProducts">
    ...
    <v-slide-group v-model="model" class="pa-4">
      <v-slide-group-item v-for="department in data.departments" :key="department.id" v-slot="{ isSelected, toggle }">
        <v-card @click="toggle">
            ...
            <v-slide-group-item v-for="(product, productIndex) in department.products" :key="product.id"
              ...
              <v-card class="ma-4" height="580" @click="toggle">
                ...
                <v-card-text>
                  ...
                  <div>Category: {{ department.categories[0].categories_id.name }}</div>
                </v-card-text>
