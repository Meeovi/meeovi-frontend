    ...
    <div>
        ...
        <SfTable class="sf-table--bordered table desktop-only">
            ...
            <SfTableRow v-for="(product, index) in products" :key="index" class="table__row">
                <SfTableData class="table__image">
                    <SfImage :src="addBasePath(cartGetters.getItemImage(product))"
                ...
                <SfTableData class="table__data"
                    v-for="(value, key) in cartGetters.getItemAttributes(product, ['size', 'color'])" :key="key">
                    {{ value }}
                </SfTableData>
                <SfTableData class="table__data">{{ cartGetters.getItemQty(product) }}</SfTableData>
                <SfTableData class="table__data price">
                    <SfPrice :regular="$n(cartGetters.getItemPrice(product).regular, 'currency')"
        ...
        <div class="summary">
            <div class="summary__group">
                ...
                <SfProperty name="Total price" :value="$n(totals.total, 'currency')"
                ...
                <div v-e2e="'payment-summary-buttons'" class="summary__action">
                    ...
                    <SfButton :disabled="loading || !isPaymentReady || !terms" class="summary__action-button"
                        @click="processOrder">
                        {{ $t('Make an order') }}
                    </SfButton>
                </div>
        ...
        setup(props, context) {
            ...
            const processOrder = async () => {
                ...
                const thankYouPath = {
                    name: 'thank-you',
                    query: {
                        order: orderGetters.getId(order.value)
                    }
                };
