...
<script setup>
    ...
    import { overview, aboutQuery, legalQuery, companyQuery, copyrightQuery } from '~/graphql/CMS/queries/menus/footer.gql'
  ...
  const {
    data
  } = useAsyncQuery(aboutQuery);

  const {
    data: legal
  } = useAsyncQuery(legalQuery);

  const {
    data: company
  } = useAsyncQuery(companyQuery);

  const {
    data: copyrightfooter
  } = useAsyncQuery(copyrightQuery);
