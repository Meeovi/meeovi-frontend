<template>
  ...
  <script>
import { postComment } from '~/composables/social/comments';
  ...
  methods: {
    async submitComment() {
      if (this.commentText.trim() !== '') {
        const success = await postComment({ text: this.commentText });
        if (success) {
          // Optionally, update comment list after successful post
