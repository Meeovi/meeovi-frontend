<template>
    <div class="flex flex-wrap gap-4 lg:gap-6 lg:flex-nowrap">
        <div v-for="({ icon, title, description, buttonText, isDisabled }, index) in cardDetails"
            :key="`${title}-${index}`"
            ...
            <div class="p-4 flex flex-col items-center">
              ...
              { 'text-disabled-700': isDisabled },
                ...
                <SfButton size="sm" variant="secondary" :disabled="isDisabled" class="mt-auto">{{ buttonText }}
                </SfButton>
            </div>
        </div>
    </div>
