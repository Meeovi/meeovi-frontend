<template>
    <div>
        <section v-for="categories in data.categories.items" :key="categories" data-bs-version="5.1" class="features19 cid-tAGUZsWaoz mbr-parallax-background" id="features12-62" :style="`background-image: url(${categories.image});`">
...
<div class="container">
    <h2 class="mbr-section-title align-left mbr-fonts-style mbr-bold display-2">
        <strong>Hottest Sounds in Meeovi {{ categories.name }}</strong></h2>
...
const query = gql`
query {
    categories (filters: {ids: {in: "42"}}) {
      items {
        name
        image
      }
    }
    products(filter: {category_id: {eq: "42"}}, pageSize: 3){
    items {
      uid
      name
      categories {
