...
const query = gql`
  query {
  collections (options: {topLevelOnly: false, filter: {name: {eq: "Books"}}}){
    items {
      name
      featuredAsset {
        id
...
const departments = await getItems({ collection: "departments", params: {filter: {name: {_eq: "Books"}}}, limit: 6 });*/
</script>