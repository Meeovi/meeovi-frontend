<template>
    <div>
        <v-sheet class="mx-auto sliderProducts">
            <h4>News Articles worth reading..</h4>
            <v-sheet class="mx-auto" elevation="0" color="transparent">
                <v-slide-group v-model="model" class="pa-4" prev-icon="fas fa-arrow-left" next-icon="fas fa-arrow-right"
                    ...
                    <v-slide-group-item v-slot="{ isSelected, toggle, selectedClass }">
                        <v-row dense v-for="news in data?.productCategories?.nodes" :key="news">
                            <v-col v-for="items in news?.nodes" :key="items" cols="4">
                                <v-card class="mx-auto" max-width="400">
                                    ...
                                    <v-card-text>
                                        ...
                                        <div>{{ items?.excerpt }}</div>
                                    </v-card-text>

                                    <v-card-actions>
                                        <v-btn color="orange" text="Read"></v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-col>
                        </v-row>
...
<script setup>
    const query = gql `
query NewQuery {
  productCategories(where: {name: "News"}) {
    nodes {
      id
      name
      children {
