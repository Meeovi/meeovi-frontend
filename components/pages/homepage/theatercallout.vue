<template>
  <div>
    <section data-bs-version="5.1" class="features1 cid-tAGULGk3ZD" id="features1-60" v-for="callout in data?.pages?.nodes" :key="callout">
...
<script setup>
  const query = gql `
query NewQuery {
  pages(where: {name: "Theater Callout"}) {
    nodes {
      id
      title
      content
