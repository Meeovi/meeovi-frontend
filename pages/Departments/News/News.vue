<template>
    <div class="contentPage">
        <v-card class="lowerbar">
            <v-tabs center-active v-for="departments in data.departments" :key="departments">
                <h5>Meeovi {{ departments.name }}</h5>
                <v-tab><a :href="`/departments/${departments.id}`">All</a></v-tab>
                <v-tab><a
                        :href="`/categories/${departments.categories.categories_id.id}`">{{ departments.categories.categories_id.name }}</a>
                </v-tab>
            </v-tabs>
        </v-card>
        ...
        <v-row class="productPage">
            <v-col cols="3" v-for="articles in data.departments.articles" :key="articles">
                <a href="">
            ...
            <v-slide-group v-model="model" class="pa-4" center-active show-arrows>
                <v-slide-group-item v-for="shops in data.departments.shops" :key="shops.id"
                    ...
                    <a :href="`/product/${shops.id}`">
                        <v-card class="ma-4" height="580" width="250" @click="toggle">
                            <img class="align-end text-white" height="280"
                                :src="`${url}assets/${shops.media.filename_disk}`" :alt="shops.name" cover />
