<template>
    <div class="contentPage">
        <profilebar />
        <v-row>
            <v-col cols="12" v-for="orders in orders" :key="orders">
                <v-sheet class="mx-auto sliderProducts">
                    <v-toolbar color="transparent">
                        <v-toolbar-title>{{ orders.order_number }}</v-toolbar-title>
                        <p>Total Price: {{ orders.total_price }}</p>
                        <v-btn variant="text" color="orange">Refund Order</v-btn>
                    </v-toolbar>
                    <v-slide-group v-model="model" class="pa-4">
                        <v-slide-group-item v-for="orders in orders" :key="orders.id"
                            v-slot="{ isSelected, toggle }">
                            <v-card @click="toggle">
                                <v-slide-group v-model="productModel" class="pa-4">
                                    <v-slide-group-item v-for="(product, productIndex) in orders.items"
                                        ...
                                        <v-card class="ma-4" height="580" @click="toggle">
                                            ...
                                            <v-card-text>
                                                ...
                                                <div>Category: {{ orders.categories[0].categories_id.name }}</div>
                                            </v-card-text>
