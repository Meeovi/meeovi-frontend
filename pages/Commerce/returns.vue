<template>
    <div class="contentPage">
        <profilebar />
        <v-row>
            <v-col cols="12" v-for="returns in returns" :key="returns">
                <v-sheet class="mx-auto sliderProducts">
                    <v-toolbar color="transparent">
                        <v-toolbar-title>Return Number {{ returns.return_number }}</v-toolbar-title>
                        <p>Status: {{ returns.status }}</p>
                    </v-toolbar>
                    <v-slide-group v-model="model" class="pa-4">
                        <v-slide-group-item v-for="returns in returns" :key="returns.id"
                            v-slot="{ isSelected, toggle }">
                            <v-card @click="toggle">
                                <v-slide-group v-model="productModel" class="pa-4">
                                    <v-slide-group-item v-for="(product, productIndex) in returns.items"
                                        ...
                                        <v-card class="ma-4" height="580" @click="toggle">
                                            ...
                                            <v-card-text>
                                                ...
                                                <div>Category: {{ returns.categories[0].categories_id.name }}</div>
                                            </v-card-text>
