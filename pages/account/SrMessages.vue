<template>
    <div id="messages" role="alert">
        <span v-for="message in splitMessages" :key="message">
            > {{ message.content }}<a v-if="message.paymentIntent"
                :href="addDashboardLinks(message.paymentIntent)">{{ message.paymentIntent }}</a>
            <br>
        </span>
    </div>
</template>
...
<script setup>
    ...
    const splitMessages = computed(() => {
        return messagesRef.value.map((x) => {
            ...
            return {
                ...(paymentIntentMatch && {
                    paymentIntent: paymentIntentMatch[0]
                }),
